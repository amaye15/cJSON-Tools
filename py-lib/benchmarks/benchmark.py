#!/usr/bin/env python3
"""
Comprehensive benchmarks for the cJSON-Tools Python bindings.

This script benchmarks the performance of the cJSON-Tools Python bindings
...
def benchmark_flatten_batch(json_objects, use_threads=False, num_threads=0, num_runs=5):
    ...
    for _ in range(num_runs):
        start_time = time.time()
        flatten_json_batch(
            json_objects, use_threads=use_threads, num_threads=num_threads
        )
        total_time += time.time() - start_time
...
def run_benchmarks(sizes, num_runs=3):
    ...
    for size in sizes:
        ...
        flatten_batch_st_time = benchmark_flatten_batch(
            json_objects, use_threads=False, num_runs=num_runs
        )

        # Flatten batch (multi-threaded, auto)
        flatten_batch_mt_auto_time = benchmark_flatten_batch(
            json_objects, use_threads=True, num_runs=num_runs
        )

        # Flatten batch (multi-threaded, specific)
        flatten_batch_mt_spec_time = benchmark_flatten_batch(
            json_objects, use_threads=True, num_threads=cpu_count, num_runs=num_runs
        )
        ...
        schema_batch_st_time = benchmark_schema_batch(
            json_objects, use_threads=False, num_runs=num_runs
        )

        # Schema batch (multi-threaded, auto)
        schema_batch_mt_auto_time = benchmark_schema_batch(
            json_objects, use_threads=True, num_runs=num_runs
        )

        # Schema batch (multi-threaded, specific)
        schema_batch_mt_spec_time = benchmark_schema_batch(
            json_objects, use_threads=True, num_threads=cpu_count, num_runs=num_runs
        )
...
def save_results_to_markdown(results, filename):
    ...
    with open(filename, "w") as f:
        ...
        avg_flatten_batch_st_speedup = sum(
            r["flatten_batch_st_speedup"] for r in results
        ) / len(results)
        avg_flatten_batch_mt_auto_speedup = sum(
            r["flatten_batch_mt_auto_speedup"] for r in results
        ) / len(results)
        avg_flatten_batch_mt_spec_speedup = sum(
            r["flatten_batch_mt_spec_speedup"] for r in results
        ) / len(results)
        avg_schema_batch_st_speedup = sum(
            r["schema_batch_st_speedup"] for r in results
        ) / len(results)
        avg_schema_batch_mt_auto_speedup = sum(
            r["schema_batch_mt_auto_speedup"] for r in results
        ) / len(results)
        avg_schema_batch_mt_spec_speedup = sum(
            r["schema_batch_mt_spec_speedup"] for r in results
        ) / len(results)

        f.write("### Average Speedups\n\n")
        f.write(
            f"- Flatten Batch (Single-Threaded): {avg_flatten_batch_st_speedup:.2f}x\n"
        )
        f.write(
            f"- Flatten Batch (Multi-Threaded, Auto): {avg_flatten_batch_mt_auto_speedup:.2f}x\n"
        )
        f.write(
            f"- Flatten Batch (Multi-Threaded, Specific): {avg_flatten_batch_mt_spec_speedup:.2f}x\n"
        )
        f.write(
            f"- Schema Batch (Single-Threaded): {avg_schema_batch_st_speedup:.2f}x\n"
        )
        f.write(
            f"- Schema Batch (Multi-Threaded, Auto): {avg_schema_batch_mt_auto_speedup:.2f}x\n"
        )
        f.write(
            f"- Schema Batch (Multi-Threaded, Specific): {avg_schema_batch_mt_spec_speedup:.2f}x\n\n"
        )
        ...
        if avg_flatten_batch_st_speedup > 1:
            f.write(
                "- Batch processing is beneficial for flattening operations, with an average speedup of "
        ...
        if avg_schema_batch_st_speedup > 1:
            f.write(
                "- Batch processing is beneficial for schema generation, with an average speedup of "
                f"{avg_schema_batch_st_speedup:.2f}x over single-object processing.\n"
            )
        else:
            f.write(
                "- Batch processing does not provide significant benefits for schema generation.\n"
            )

        # Determine if multi-threading is beneficial
        if avg_flatten_batch_mt_auto_speedup > avg_flatten_batch_st_speedup:
            f.write(
                "- Multi-threading (auto) improves flattening performance by "
                f"{(avg_flatten_batch_mt_auto_speedup / avg_flatten_batch_st_speedup - 1) * 100:.1f}% "
                f"compared to single-threaded batch processing.\n"
            )
        else:
            f.write(
                "- Multi-threading (auto) does not improve flattening performance compared to "
                "single-threaded batch processing.\n"
            )

        if avg_flatten_batch_mt_spec_speedup > avg_flatten_batch_st_speedup:
            f.write(
                "- Multi-threading (specific) improves flattening performance by "
                f"{(avg_flatten_batch_mt_spec_speedup / avg_flatten_batch_st_speedup - 1) * 100:.1f}% "
                f"compared to single-threaded batch processing.\n"
            )
        else:
            f.write(
                "- Multi-threading (specific) does not improve flattening performance compared to "
                "single-threaded batch processing.\n"
            )

        if avg_schema_batch_mt_auto_speedup > avg_schema_batch_st_speedup:
            f.write(
                "- Multi-threading (auto) improves schema generation performance by "
                f"{(avg_schema_batch_mt_auto_speedup / avg_schema_batch_st_speedup - 1) * 100:.1f}% "
                f"compared to single-threaded batch processing.\n"
            )
        else:
            f.write(
                "- Multi-threading (auto) does not improve schema generation performance compared to "
                "single-threaded batch processing.\n"
            )

        if avg_schema_batch_mt_spec_speedup > avg_schema_batch_st_speedup:
            f.write(
                "- Multi-threading (specific) improves schema generation performance by "
                f"{(avg_schema_batch_mt_spec_speedup / avg_schema_batch_st_speedup - 1) * 100:.1f}% "
                f"compared to single-threaded batch processing.\n"
            )
        else:
            f.write(
                "- Multi-threading (specific) does not improve schema generation performance compared to "
                "single-threaded batch processing.\n"
            )
        ...
        if avg_flatten_batch_mt_spec_speedup > avg_flatten_batch_mt_auto_speedup:
            f.write(
                f"- Using a specific thread count ({multiprocessing.cpu_count()}) is more efficient than "
                f"auto-detection for flattening operations.\n"
            )
        else:
            f.write(
                "- Auto-detection of thread count is more efficient than using a specific thread count "
        ...
        best_flatten_size = max(
            results, key=lambda r: r["flatten_batch_mt_auto_speedup"]
        )["size"]
        best_schema_size = max(
            results, key=lambda r: r["schema_batch_mt_auto_speedup"]
        )["size"]

        f.write(
            f"- The optimal batch size for flattening operations is around {best_flatten_size} objects.\n"
        )
        f.write(
            f"- The optimal batch size for schema generation is around {best_schema_size} objects.\n"
        )
...
def plot_results(results, filename):
    ...
    axs[0, 0].plot(
        sizes, [r["flatten_batch_st"] for r in results], "s-", label="Batch ST"
    )
    axs[0, 0].plot(
        sizes,
        [r["flatten_batch_mt_auto"] for r in results],
        "^-",
        label="Batch MT Auto",
    )
    axs[0, 0].plot(
        sizes,
        [r["flatten_batch_mt_spec"] for r in results],
        "d-",
        label="Batch MT Spec",
    )
    ...
    axs[0, 1].plot(
        sizes, [r["schema_batch_st"] for r in results], "s-", label="Batch ST"
    )
    axs[0, 1].plot(
        sizes, [r["schema_batch_mt_auto"] for r in results], "^-", label="Batch MT Auto"
    )
    axs[0, 1].plot(
        sizes, [r["schema_batch_mt_spec"] for r in results], "d-", label="Batch MT Spec"
    )
    ...
    axs[1, 0].plot(
        sizes, [r["flatten_batch_st_speedup"] for r in results], "s-", label="Batch ST"
    )
    axs[1, 0].plot(
        sizes,
        [r["flatten_batch_mt_auto_speedup"] for r in results],
    ...
    axs[1, 1].plot(
        sizes, [r["schema_batch_st_speedup"] for r in results], "s-", label="Batch ST"
    )
    axs[1, 1].plot(
        sizes,
        [r["schema_batch_mt_auto_speedup"] for r in results],
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Benchmark cJSON-Tools Python bindings"
    )
    parser.add_argument(
        "--sizes",
        type=int,
    ...
    parser.add_argument(
        "--runs", type=int, default=3, help="Number of runs for each benchmark"
    )
    parser.add_argument(
        "--output",
        type=str,

...
class ComprehensiveBenchmark:
    ...
    def benchmark_jsontools_builder_complete(
        self, data: Dict, iterations: int = 50
    ) -> Dict[str, List[float]]:
        ...
        for _ in range(iterations):
            ...
            result = (
                ...
                .replace_keys(
                    r"^session\.tracking\..*\.timeMs$", "session.unified.time"
                )
        ...
        for _ in range(iterations):
            start_time = time.perf_counter()
            result = JsonToolsBuilder().add_json(json_str).flatten().build()
            end_time = time.perf_counter()
            times.append(end_time - start_time)
    ...
    def benchmark_individual_functions(
        self, data: Dict, iterations: int = 100
    ) -> Dict[str, List[float]]:
    ...
    def benchmark_batch_operations(
        self, data: Dict, iterations: int = 20
    ) -> Dict[str, List[float]]:
    ...
    def run_comprehensive_benchmark(self):
        ...
        for size in sizes:
            ...
            for test_name, times in builder_results.items():
                mean_time = self.print_results(
                    f"JsonToolsBuilder - {test_name} ({size})", times, dataset_size
                )
            ...
            for test_name, times in individual_results.items():
                mean_time = self.print_results(
                    f"Individual - {test_name} ({size})", times, dataset_size
                )
            ...
            for test_name, times in batch_results.items():
                mean_time = self.print_results(
                    f"Batch - {test_name} ({size})", times, dataset_size
                )
    ...
    def _print_performance_insights(
        self, size: str, results: Dict[str, float], dataset_size: int
    ):
        ...
        if (
            "builder_flattening_only" in results
            and "individual_flatten_json" in results
        ):
            ...
            print(
                f"   JsonToolsBuilder flattening is {speedup:.1f}x vs individual function"
            )

        # Compare operations with and without flattening
        if (
            "builder_complete_operations" in results
            and "builder_complete_with_flattening" in results
        ):
    ...
    def _print_overall_summary(self, all_results: Dict[str, Dict[str, float]]):
        ...
        print(
            f"      - Flattening: JsonToolsBuilder is optimized for complex operations"
        )
        print(f"      - Individual functions are faster for single operations")
        print(
            f"      - JsonToolsBuilder TRUE single-pass excels with multiple operations"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Comprehensive cJSON-Tools Benchmark Suite"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Run specific dataset size only",
    )

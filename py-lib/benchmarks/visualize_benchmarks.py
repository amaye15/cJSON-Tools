...
def read_benchmark_results(file_path):
    ...
    flatten_lines = re.findall(
        r"\| (.*?) \| (.*?) \| (.*?) \| (.*?) \|",
        re.search(r"## Flattening Performance.*?## Schema", content, re.DOTALL).group(
            0
        ),
    )
    ...
    schema_section = re.search(
        r"## Schema Generation Performance.*?##", content, re.DOTALL
    )
    ...
    schema_lines = re.findall(
        r"\| (.*?) \| (.*?) \| (.*?) \| (.*?) \|", schema_section.group(0)
    )

    if len(flatten_lines) < num_sizes + 1 or len(schema_lines) < num_sizes + 1:
        print(
            f"Could not parse benchmark results. Found {len(flatten_lines)} flatten lines and {len(schema_lines)} schema lines"
        )
        return None
...
def create_charts(flatten_results, schema_results):
    ...
    plt.bar(
        x - width / 2,
        flatten_results["single_threaded"],
        width,
        label="Single-threaded",
    )
    plt.bar(
        x + width / 2, flatten_results["multi_threaded"], width, label="Multi-threaded"
    )
    ...
    plt.subplot(2, 2, 3)
    plt.bar(
        x - width / 2, schema_results["single_threaded"], width, label="Single-threaded"
    )
    plt.bar(
        x + width / 2, schema_results["multi_threaded"], width, label="Multi-threaded"
    )
    plt.xlabel("File Size")
    plt.ylabel("Time (seconds)")

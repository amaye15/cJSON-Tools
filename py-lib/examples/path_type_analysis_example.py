...
def main():
    ...
    paths_with_types = cjson_tools.get_flattened_paths_with_types(json.dumps(complex_data))
    ...
    print(f"Path type analysis: {type_analysis_time:.6f}s ({len(large_paths_result)} unique paths)")
    print(f"Traditional flattening: {flattening_time:.6f}s ({len(large_flattened_result)} total paths)")
    print(f"Type analysis is {flattening_time/type_analysis_time:.1f}x faster for path discovery!")

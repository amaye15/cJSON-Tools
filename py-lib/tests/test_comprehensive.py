...
class TestCJsonToolsComprehensive(unittest.TestCase):
    ...
    def test_flatten_json_batch(self):
        ...
        # Test multi-threaded
        results_mt = flatten_json_batch(self.json_batch, use_threads=True, num_threads=2)
...
def run_performance_benchmark():
    ...
    print(f"Batch flattening (100 items): {batch_flatten_time:.2f}ms ({batch_flatten_time/100:.3f}ms per item)")
    print(f"Batch schema generation (100 items): {batch_schema_time:.2f}ms ({batch_schema_time/100:.3f}ms per item)")
